...
//    //delay(1000);
    ...
    lv_timer_create(update_chart_adc_33_34,500,NULL);
     lv_timer_create(led_fan,200,pSW);
    ...
    lv_obj_add_event_cb(objects.onpwm_sw,event_handler,LV_EVENT_VALUE_CHANGED,NULL);
...
void loop()
{
  lv_bar_set_value(objects.adc33bar,analogRead(adc33),LV_ANIM_OFF);
  int bar1_val =lv_bar_get_value(objects.adc33bar);
  char buffer[5];
  sprintf(buffer,"%d",bar1_val);
  lv_label_set_text(objects.adc33val,buffer);

  

  lv_bar_set_value(objects.adc34bar,analogRead(adc34),LV_ANIM_OFF);
  int bar2_val2 =lv_bar_get_value(objects.adc34bar);
  char buffer1[5];
  sprintf(buffer1,"%d",bar2_val2);
  lv_label_set_text(objects.adc34val,buffer1);
  
  int pwmval =(bar1_val/4095.0)*255;
  lv_led_set_brightness(objects.led33,pwmval);
